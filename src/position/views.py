...
class PositionViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        new_parent = request.data.pop(
            instance.parent_attribute, (parent.id if parent is not None else None)
        )
        ...
        try:
            if change_parent:
                ...
                parent_model = instance._meta.get_field(
                    instance.parent_attribute
                ).related_model
                max_position = (
                    type(instance)
                    .objects.filter(**{instance.parent_attribute: new_parent})
                    .aggregate(Max("position"))["position__max"]
                )
                max_position = (max_position or 0) + 1
                new_parent_instance = parent_model.objects.get(id=new_parent)
                type(instance).swap_parent(
                    instance=instance,
            ...
            if change_position:
                ...
                target_instance = type(instance).objects.get(
                    **{instance.parent_attribute: new_parent}, position=new_position
                )
                type(instance).swap_positions(
                    instance1=instance, instance2=target_instance
                )
        except ValueError as e:
            raise PositionException(
                "Invalid value type", status.HTTP_400_BAD_REQUEST, e
            )
        except IntegrityError as e:
            raise PositionException(
                "Request did not end successfully", status.HTTP_409_CONFLICT, e
            )
        except parent_model.DoesNotExist:
            type(instance).swap_parent(
                instance=instance, new_parent=None, new_position=max_position
            )
